                    ...
                    <div className='bg-zinc-300 h-fit col-span-2 p-5 rounded'>
                        {comments.slice(page * num, page * num + num).map(comment => (
                            <div className='mb-5 flex items-center gap-x-4' key={comment._id}>
                                ...
                                {keycloak.hasRealmRole('admin') && <button className='hover:bg-zinc-200 outline outline-3 outline-rose-500 p-2 rounded-lg' onClick={() => dispatch(deleteComment(comment._id))}>{polish ? 'USUŃ' : 'DEL'}</button>}
                            </div>
