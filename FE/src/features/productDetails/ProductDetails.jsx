                ...
                <div className='bg-zinc-200 mt-10 p-6 container flex flex-col justify-cneter items-center gap-y-6 divide-y-2 divide-zinc-400 rounded'>
                    ...
                    <div className='px-32 py-6 grid grid-cols-3 gap-x-32'>
                        ...
                        <div className='w-fit flex flex-col justify-between items-end'>
                            ...
                            <div className='flex items-center gap-x-6'>
                                {keycloak.hasRealmRole('admin') && <button className='bg-zinc-700 hover:bg-zinc-600 h-fit text-zinc-50 p-2 rounded' onClick={() => navigate('/deliveryOptions')}>{polish ? 'Opcje dostawy' : 'Delivery options'}</button>}
                    ...
                    <div className='text-xl w-full flex justify-between'>
                        <div className='flex items-end gap-x-6'>
                            {keycloak.hasRealmRole('admin') && <button className='bg-rose-700 hover:bg-zinc-600 text-zinc-50 p-2 rounded' onClick={() => { dispatch(deleteProduct(productId)); navigate('/') }}>{polish ? 'Usuń' : 'Delete'}</button>}
                            {keycloak.hasRealmRole('admin') && <button className='bg-zinc-700 hover:bg-zinc-600 text-zinc-50 p-2 rounded' onClick={() => navigate(`/${productId}/edit`)}>{polish ? 'Edytuj' : 'Edit'}</button>}
                        </div>
                        <div className='flex items-end gap-x-6'>
                            {keycloak.hasRealmRole('admin') && <button className='bg-zinc-700 hover:bg-zinc-600 text-zinc-50 p-2 rounded' onClick={() => navigate(`/${productId}/scores`)}>{polish ? 'Oceny' : 'Scores'}</button>}
